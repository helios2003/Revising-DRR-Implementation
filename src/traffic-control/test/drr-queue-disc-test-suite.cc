...
class DRRQueueDiscNoSuitableFilter : public TestCase
{
  public:
    ...
    ~DRRQueueDiscNoSuitableFilter() override;
...
void
DRRQueueDiscNoSuitableFilter::DoRun()
{
    // Packets that cannot be classified by the available filters should be dropped
    Ptr<DRRQueueDisc> queueDisc =
        CreateObjectWithAttributes<DRRQueueDisc>("ByteLimit", UintegerValue(1000));
    ...
    NS_TEST_ASSERT_MSG_EQ(queueDisc->GetNQueueDiscClasses(),
                          1,
                          "One flow queue should have been created");
...
class DRRQueueDiscIPFlowsSeparationAndByteLimit : public TestCase
{
  public:
    ...
    ~DRRQueueDiscIPFlowsSeparationAndByteLimit() override;
...
void
DRRQueueDiscIPFlowsSeparationAndByteLimit::DoRun()
{
    Ptr<DRRQueueDisc> queueDisc =
        CreateObjectWithAttributes<DRRQueueDisc>("ByteLimit", UintegerValue(2500));
    ...
    NS_TEST_EXPECT_MSG_EQ((!item), true, "Veryfing Dequeue on empty queue returns 0");
 ...
 *
 * \brief Test 1: Running the DRR algorithm when packets are in one flow
 */
class DRRQueueDiscDeficitVariableSizeSameFlow : public TestCase
{
  public:
    DRRQueueDiscDeficitVariableSizeSameFlow();
    ~DRRQueueDiscDeficitVariableSizeSameFlow() override;
...
/**
 * Adds packets into the same flow
 * \param queue
 * \param hdr
 * \param size
 */
void
DRRQueueDiscDeficitVariableSizeSameFlow::AddPacket(Ptr<DRRQueueDisc> queue,
                                                   Ipv4Header hdr,
                                                   uint32_t size)
{
...
void
DRRQueueDiscDeficitVariableSizeSameFlow::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(queueDisc->GetQueueDiscClass(0)->GetQueueDisc()->GetNBytes(),
                          520,
                          "unexpected number of bytes in the first flow queue");
    ...
    NS_TEST_ASSERT_MSG_EQ(flow1->GetStatus(),
                          DRRFlow::ACTIVE,
                          "the first flow must be in the list of active queues");
    // Dequeue a packet
    queueDisc->Dequeue();
    NS_TEST_ASSERT_MSG_EQ(queueDisc->QueueDisc::GetNPackets(),
                          0,
    ...
    NS_TEST_ASSERT_MSG_EQ(flow1->GetStatus(),
                          DRRFlow::INACTIVE,
                          "the first flow must be in the list of inactive queues");
    ...
    NS_TEST_ASSERT_MSG_EQ(
        flow1->GetStatus(),
        DRRFlow::ACTIVE,
        "the first flow must be in the list of active queues since there is one packet left");
 ...
 * \ingroup traffic-control-test
 *
 * \brief Test 2: Putting into multiple flows based on the header and running the DRR algorithm
 */
class DRRQueueDiscDeficitVariableSizeDifferentFlow : public TestCase
{
  public:
    DRRQueueDiscDeficitVariableSizeDifferentFlow();
    ~DRRQueueDiscDeficitVariableSizeDifferentFlow() override;

  private:
    void DoRun() override;
    /**
     * Adds packets into the queue
     * \param queue
     * \param hdr
     * \param size
     */
...
void
DRRQueueDiscDeficitVariableSizeDifferentFlow::AddPacket(Ptr<DRRQueueDisc> queue,
                                                        Ipv4Header hdr,
                                                        uint32_t size)
{
    Ptr<Packet> p = Create<Packet>(size);
...
void
DRRQueueDiscDeficitVariableSizeDifferentFlow::DoRun()
{
    Ptr<DRRQueueDisc> queueDisc = CreateObjectWithAttributes<DRRQueueDisc>();
    // Ptr<DRRIpv6PacketFilter> ipv6Filter = CreateObject<DRRIpv6PacketFilter> ();
    Ptr<DRRIpv4PacketFilter> ipv4Filter = CreateObject<DRRIpv4PacketFilter>();
    ...
    NS_TEST_ASSERT_MSG_EQ(flow2->GetStatus(),
                          DRRFlow::ACTIVE,
                          "the second flow must be in the list of active queues");
    // Dequeue a packet
    queueDisc->Dequeue();
    ...
    /// Dequeue second packet
    queueDisc->Dequeue();
    // the deficit for the second flow becomes 600 but packet is not dequeued since size is more
    // than deficit As a result now packet is dequeud again from the first flow since deficit there
    ...
    NS_TEST_ASSERT_MSG_EQ(
        flow2->GetStatus(),
        DRRFlow::ACTIVE,
        "the second flow must be in the list of active queues since there is one packet left");
    NS_TEST_ASSERT_MSG_EQ(queueDisc->QueueDisc::GetNPackets(),
                          1,
                          "unexpected number of packets in the queue disc");
    NS_TEST_ASSERT_MSG_EQ(queueDisc->GetQueueDiscClass(0)->GetQueueDisc()->GetNPackets(),
